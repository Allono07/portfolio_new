...
· Technologies: Kafka for event synchronization, Git, Docker, Kotlin, Spring Boot, Deployed on Google Cloud`,
                ...
                proj.links.forEach(link => {
                    if (link.disabled && link.label === 'Play Store') {
                        ...
                        btn.addEventListener('click', e => {
                            ...
                            window.open('https://drive.google.com/file/d/1ZiEmzv0f8GxDXSDeSK1LIMXDie5bN1T3/view?usp=sharing', '_blank');
                        });
    ...
    // Ensure smooth scroll on nav click (for internal anchor links only)
    ...
    try {
        function generateBgShapes(count = 12) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const left = (Math.random() * 110 - 5).toFixed(2) + '%'; // allow slightly off-edge starts
                const top = (100 + Math.random() * 30).toFixed(2) + '%'; // start below viewport
                const dur = (Math.random() * 20 + 12).toFixed(2) + 's'; // 12 - 32s
                const delay = (Math.random() * -24).toFixed(2) + 's'; // negative delay for stagger
                ...
                const drift = Math.round((Math.random() - 0.5) * 140) + 'px'; // horizontal drift -70..70
                ...
                el.style.background = ''; // let CSS radial-gradient use --color
        ...
        // Choose count based on viewport width
        const chooseCount = () => {
